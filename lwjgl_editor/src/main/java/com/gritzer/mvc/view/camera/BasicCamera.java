...
public class BasicCamera implements Camera {
	...
	public boolean isInView(ViewPort bounds, ViewPort rectangle) {
		return bounds.contains(new ViewPort(
				translateXToView((float) rectangle.x),
				translateYToView((float) rectangle.y), rectangle.w * zoom,
				rectangle.h * zoom));
	}
	...
	public ViewPort translateToModel(ViewPort rect) {
		return new ViewPort(translateXToModel((float) rect.x),
				translateYToModel((float) rect.y), rect.w / zoom, rect.h / zoom);
	}
