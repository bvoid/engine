...
public class BasicPhysicalState implements PhysicalState {
  ...
  public BasicPhysicalState setDirection(double x, double y) {
    direction = normalize((new Vector(0, 0, x - position.x2, y - position.y2)));
    return this;
  }
  ...
  public BasicPhysicalState accelerate(Vector direction, double time) {
    momentum = momentum.add(normalize(direction).multiply(thrust / mass * time));
    return this;
  }
  ...
  public static Vector normalize(Vector v1) {
    ...
    v.x1 = 0;
    v.y1 = 0;
    if (v.x2 < epsilon && v.x2 > -epsilon && v.y2 < epsilon && v.y2 > -epsilon) {
      v.x2 = (1.0f);
      v.y2 = (0.0f);
    } else {
  ...
  public PhysicalState correctMomentum(double time) {
    ...
    // direction));
    momentum = momentum.add(momentum.multiply(-time)).add(normalize(direction).multiply(time));
