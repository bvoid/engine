...
public class EditorSimCanvas extends BasicCanvas {
	...
	private void rotateAndRender(PolygonBot bot) {
		final float realX = (bot.getX() + bot.getPolygon().getX() - camera
				.getX()) * camera.getZoom();
		final float realY = (bot.getY() + bot.getPolygon().getX() - camera
				.getY()) * camera.getZoom();

		render(Polygons.transform(bot.getPolygon(), realX, realY,
				bot.getTheta(), camera.getZoom()));
	}
