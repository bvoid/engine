...
public class PolygonBot implements Simulatable {
	...
	public void move(double x, double y, double time) {
		...
		physicalState.setDirection(BasicPhysicalState.normalize(physicalState
				.getMomentum()));
	...
	public void push(float degrees, float time) {
		physicalState.accelerate(
				BasicPhysicalState.getDirection(Math.toRadians(degrees)), time);
	}
