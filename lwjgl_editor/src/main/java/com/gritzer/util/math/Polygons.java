...
public class Polygons {
	...
	public static Polygon transform(Polygon p, float x, float y, float theta,
			float zoom) {
	...
	public static boolean containsInBounds(Polygon p, float x, float y) {
		...
		// rotated polygons
		final boolean contains = p.getX() <= x && p.getY() <= y
				&& (p.getX() + p.getW()) >= x && (p.getY() + p.getH()) >= y;
	...
	private static void transform(float[] originalXYPoints,
			float[] newXYPoints, int length) {
	...
	private static float[] extractPoints(Polygon p) {
		int xIndex = 0;
		int yIndex = 0;
		final float[] originalXYPoints = new float[p.getxPoints().length
				+ p.getyPoints().length];
	...
	private static void setPoints(Polygon p, float[] newPoints, float x,
			float y, float zoom) {
		int xIndex = 0;
		int yIndex = 0;
		for (int i = 0; i < newPoints.length; i++) {
